...
public class DelayedTaskExecutor {
    ...
    // 使用类似于单例的方法
    private static HashMap<MinecraftServer, DelayedTaskExecutor> instanceMap = new HashMap<>();
    /**
     * 若服务器对应的实例暂不存在，将会创建一个并基于server每刻触发的机制运作
     * @param server 需要新建实例时依赖的服务器
     * */
    public static DelayedTaskExecutor getInstance(MinecraftServer server) {
        return instanceMap.computeIfAbsent(server, DelayedTaskExecutor::new);
    }
    // 延迟单次变量，使用TreeMap获得综合最佳的添加/访问速度
    private HashMap<Integer, Collection<DelayedTask>> singularTasks;
    // 所有的定时运行任务
    private ArrayList<DelayedTask> timerTasks;
    private int currentTick;

    // constructor，初始化数据结构并记录为第一刻
    private DelayedTaskExecutor(MinecraftServer server) {
        singularTasks = new HashMap<>();
